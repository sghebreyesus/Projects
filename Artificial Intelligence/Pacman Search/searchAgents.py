        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if new_node in self.corners:
                    if new_node not in explored_corners:
                        explored_corners = explored_corners + (new_node, )
                successors.append(((new_node, explored_corners), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    while len(unexplored_corners) != 0:
        min_d = 0
        corner_index = 0
        for i in range(len(unexplored_corners)):
            distance = util.manhattanDistance(coordinates, unexplored_corners[i])
            if min_d == 0 or min_d > distance:
                min_d = distance
                corner_index = i
        heuristic += min_d
        coordinates = unexplored_corners[corner_index]
        unexplored_corners.remove(unexplored_corners[corner_index])
    return heuristic
